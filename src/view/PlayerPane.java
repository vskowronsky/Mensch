...
public class PlayerPane extends Pane{
	...
	private void initBoard() {
		...
		circles[0] = new Circle(4.5*unit, 10.5*unit, size/2, 0);
		circles[1] = new Circle(4.5*unit, 9.5*unit, size/2, 1);
	...
	private void initStreet() {
		...
		RectangleWithPos[] recStreetGreen = new RectangleWithPos[4];
		for(int i = 0; i < 4; i++) {
			recStreetGreen[i] = new RectangleWithPos ((i+1)*unit, 5*unit, size, size, i);
			...
			recStreetGreen[i].setOnMouseEntered(gui.recEnteredEventHandler);
			recStreetGreen[i].setOnMouseExited(gui.recLeftEventHandler);
		}

		Rectangle[] recStreetBlue = new Rectangle[4];
		for(int i = 0; i < 4; i++) {
			recStreetBlue[i] = new Rectangle (5*unit, (i+1)*unit, size, size);
			recStreetBlue[i].setFill(Color.FLORALWHITE);
			recStreetBlue[i].setStroke(Color.ROYALBLUE);
			recStreetBlue[i].setStrokeWidth(3);
		}

		Rectangle[] recStreetRed = new Rectangle[4];
		for(int i = 0; i < 4; i++) {
			recStreetRed[i] = new Rectangle (width-(i+2)*unit, 5*unit, size, size);
			recStreetRed[i].setFill(Color.FLORALWHITE);
			recStreetRed[i].setStroke(Color.FIREBRICK);
			recStreetRed[i].setStrokeWidth(3);
		}

		Rectangle[] recStreetYellow = new Rectangle[4];
		for(int i = 0; i < 4; i++) {
			recStreetYellow[i] = new Rectangle (5*unit,width-(i+2)*unit, size, size);
